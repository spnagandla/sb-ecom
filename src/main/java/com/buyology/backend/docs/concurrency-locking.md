...
5. **Concurrent cart quantity patches (`updateProductQuantityInCart`)**
   ...
   - This branch now applies pessimistic locks in that flow on cart/cart-item rows to prevent interleaving quantity + total updates.
...
- `CartServiceImpl.addProductToCart(...)` now:
  ...
  - Reads product stock without row-level write lock (no stock mutation in this transaction).
